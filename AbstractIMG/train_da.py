...
from models.DA_VGG_KD import A_Model, B_Model, DA_Distilling
...
def train(config, encode_X, Y, mart_train_x, mart_train_y, mart_test_x, mart_test_y):
    ...
    if config.model == 'KD':
        epoch_num = 1
        # 先训练源领域
        a_model = A_Model(len(set(Y)), np.array(encode_X), np.array(Y), epoch_num, 32).return_model()
        # eval(a_model, test_x, test_y, 64)
        b_model = B_Model().return_model()

        model = DA_Distilling(b_model, a_model, 4, 0.9)
        # model([source_img, target_img])
        ...
        data = {'target_img': np.array(mart_train_x)}
        # 由于模型覆写了train_step，导致我不知道应该怎么做不同维度的多输入，这里曲线救国一下
        for i in range(len(encode_X)//len(mart_train_x)):
            start, end = i*len(mart_train_x), (i+1)*len(mart_train_x)
            data[f'source{i}'] = np.array(encode_X[start:end])

        model.fit(data, np.array(distill_y), epochs=epoch_num, callbacks=callback, batch_size=32)
        report = eval(model, np.array(mart_test_x), np.array(mart_test_y), 32)
...
def eval(model, test_x, test_y, batch_size):
    y_pred = model.predict({'target_img':test_x}, batch_size=batch_size)
    ...
    report = sklearn_metrics.classification_report(test_y,
                                                   ...
                                                   output_dict=True,
                                                   digits=4)
    print(sklearn_metrics.classification_report(test_y,
                                                   y_pred,
                                                   output_dict=False,
                                                   digits=4))
